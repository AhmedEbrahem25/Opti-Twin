...
const factoryNames: Record<string, string> = {
  "factory-1": "EAF Workshop — Ain Sokhna",
  "factory-2": "Alexandria Port Facility",
  "factory-3": "Alamein Processing Plant",
};

const machineNames: Record<string, string> = {
  "factory-1-m1": "EAF #2 — 185t Danieli",
};
...
function getBreadcrumbs(pathname: string) {
  ...
  if (parts[0] === "factory") {
    ...
    crumbs.push({ label: factoryNames[id] ?? id, href: pathname });
  } else if (parts[0] === "machine") {
    crumbs.push({ label: "Machines", href: "/workspace" });
    crumbs.push({ label: machineNames[id] ?? id, href: pathname });
  } else if (parts[0] === "digital-twin") {
    crumbs.push({ label: "Digital Twin", href: "/workspace" });
    crumbs.push({ label: machineNames[id] ?? id, href: pathname });
  } else {
