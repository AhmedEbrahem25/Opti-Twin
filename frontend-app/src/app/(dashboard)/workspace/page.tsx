...
const factories = [
  { id: "factory-1", name: "EAF Workshop — Ain Sokhna", location: "Ain Sokhna, Egypt", status: "RUNNING" as const, machines: 1, alerts: 0, energy: 0, saved: 0, efficiency: 0, live: true },
  { id: "factory-2", name: "Alexandria Port Facility", location: "Alexandria, Egypt", status: "RUNNING" as const, machines: 4, alerts: 0, energy: 623.1, saved: 1580, efficiency: 91, live: false },
  { id: "factory-3", name: "Alamein Processing Plant", location: "El Alamein, Egypt", status: "WARNING" as const, machines: 3, alerts: 4, energy: 412.8, saved: 900, efficiency: 73, live: false },
];
...
export default function WorkspacePage() {
  ...
  const liveLatest = useTelemetryStore((s) => s.latestPoints.get("factory-1-m1"));
  const orgName = useWorkspaceStore((s) => s.currentOrg?.name ?? "Workspace");
  ...
  const xaiLogs = xaiLogsLive.length > 0 ? xaiLogsLive.slice(0, 5) : [];
  // Memoized so mock list stays stable between re-renders.
  const mockAlerts = useMemo(() => generateMockAlerts().slice(0, 3), []);
  // Prefer live crisis alerts; fall back to mock when backend is offline.
  const alerts = liveAlerts.length > 0 ? liveAlerts.slice(0, 3) : mockAlerts;

  // Project live values into the factory-1 card row.
  const liveFactories = factories.map((f) =>
    f.live
      ? {
          ...f,
          energy: liveLatest?.energyKwh ?? f.energy,
          saved: kpis.totalCostSaved,
  ...
  return (
    <div className="space-y-6 max-w-7xl">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-text-tertiary mt-0.5">
            {orgName} — Overview {isConnected ? "" : "(offline)"}
          </p>
        </div>
      ...
      {/* Live Price Strip */}
      {livePrice && (
        <div className="flex items-center gap-3 px-4 py-2 rounded-xl bg-bg-300 border border-border text-xs">
          <Zap size={13} className={livePrice.is_peak ? "text-danger" : "text-success"} />
          <span className="font-semibold text-text-primary">
            {livePrice.price_egp_kwh.toFixed(3)} EGP/kWh
          </span>
          <Badge variant={livePrice.is_peak ? "danger" : "success"} size="sm">
            {livePrice.tariff_class}
          </Badge>
          <span className="text-text-tertiary">{livePrice.label}</span>
          {livePrice.is_dr_event && (
            <Badge variant="warning" pulse size="sm">DR Event Active</Badge>
          )}
          {drSnapshot?.active_event && (
            <span className="text-text-tertiary ml-auto">
              DR payments today: <span className="text-success font-medium">{drSnapshot.total_payments_today.toFixed(0)} EGP</span>
            </span>
          )}
          <span className="text-text-muted ml-auto text-[10px]">src: {livePrice.price_source}</span>
        </div>
      )}
