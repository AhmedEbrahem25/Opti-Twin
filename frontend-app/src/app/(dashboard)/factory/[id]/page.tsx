...
import { generateMockMachines } from "@/services/mock-data";

export default function FactoryPage({ params }: { params: Promise<{ id: string }> }) {
  const machines = generateMockMachines("factory-1");
  ...
  return (
    <div className="space-y-6 max-w-7xl">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-text-primary font-[family-name:var(--font-display)] tracking-tight">
            Cairo Industrial Complex
          </h1>
          <p className="text-sm text-text-tertiary mt-0.5">Cairo, Egypt — 5 machines across 2 production lines</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard label="Active Machines" value={5} suffix="/5" icon={<Activity size={16} />} accentColor="var(--color-cyan)" />
        <KPICard label="Energy Usage" value={847.3} suffix=" kWh" trend={-8.2} icon={<Zap size={16} />} accentColor="var(--color-warning)" />
        <KPICard label="Cost Saved Today" value={2340} suffix=" EGP" trend={18.3} icon={<DollarSign size={16} />} accentColor="var(--color-success)" />
        <KPICard label="Efficiency Score" value={87} suffix="%" trend={3.1} icon={<Gauge size={16} />} accentColor="var(--color-accent)" />
      </div>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="space-y-2 flex-1">
                          <div className="flex items-center justify-between text-xs">
                            ...
                            <span className="text-text-primary font-mono">{m.metrics.temperature?.toFixed(1)}°C</span>
                          </div>
                          <div className="flex items-center justify-between text-xs">
                            <span className="text-text-tertiary">Energy</span>
                            <span className="text-text-primary font-mono">{m.metrics.energyKwh?.toFixed(1)} kWh</span>
                          </div>
                          {m.metrics.rpm && (
                            <div className="flex items-center justify-between text-xs">
                        ...
                        <div className="ml-4">
                          <MetricRing value={m.healthScore} size={56} strokeWidth={4} label="Health" />
                        </div>
                      </div>
                      {m.status === "WARNING" && (
